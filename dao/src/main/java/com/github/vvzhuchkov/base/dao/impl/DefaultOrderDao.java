...
public class DefaultOrderDao implements OrderDao {
    ...
    public void saveOrder(Order order) {
        final String sql = "insert into base.order(login, id, pickup, dropoff) values(?,?,?,?)";
    ...
    public List<Order> getAllOrders() {
        try (Connection connection = DataSource.getInstance().getConnection();
            ...
            while (rs.next()) {
                ...
                System.out.println(order.getPickup());
            }
            return listOfOrders;
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    public Order getOrderById(Long id) {
        try (Connection connection = DataSource.getInstance().getConnection();
             PreparedStatement ps = connection.prepareStatement("select base.order.*, base.car.*, base.offer.* from base.order join base.car on order.id = car.id join base.offer on car.id = offer.id where base.order.id = ?")) {
            ps.setLong(1, id);
